...
import { Compass, Search, ArrowUpDown, RotateCcw, Filter, MapPin, Camera, Film, Wand2, ShieldCheck } from 'lucide-react';
      ...
      <section className="relative py-16 sm:py-20 px-4 sm:px-6 lg:px-8 border-b border-sky-500/15 overflow-hidden">
        <div className="max-w-7xl mx-auto space-y-4 relative z-10">
          ...
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-display font-extrabold tracking-tight text-white">
            EXPLORE ALL <span className="text-gradient-cyan text-glow-cyan">CREATIVE TALENT</span>
          </h1>
          <p className="text-xs sm:text-sm text-slate-300 max-w-2xl leading-relaxed">
            Search top photographers, cinematographers, photo editors, video colorists, and studios across India and global destinations.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6 relative z-10">
        <div className="p-5 sm:p-6 rounded-3xl glass-panel border border-sky-500/25 space-y-4 shadow-xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            >
              <option value="all" className="bg-midnight-950">All Disciplines</option>
              <option value="photographer" className="bg-midnight-950">Photographers</option>
              <option value="videographer" className="bg-midnight-950">Cinematographers / Video</option>
              <option value="editor" className="bg-midnight-950">Editors & Colorists</option>
              <option value="wedding" className="bg-midnight-950">Wedding Specialists</option>
            ...
            >
              ...
              <option value="Jaipur" className="bg-midnight-950">Jaipur / Udaipur</option>
              <option value="Hyderabad" className="bg-midnight-950">Hyderabad</option>
              <option value="Kolkata" className="bg-midnight-950">Kolkata</option>
              <option value="Dubai" className="bg-midnight-950">Dubai</option>
            </select>
            ...
            >
              ...
              <option value="luxury" className="bg-midnight-950">Elite / Studio (₹35,000+)</option>
            </select>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-white/10 text-xs">
            <div className="flex items-center gap-2">
              ...
              {(search || discipline !== 'all' || cityFilter !== 'all' || priceFilter !== 'all' || ratingFilter !== 'all') && (
                <button
                  onClick={() => {
                    setSearch('');
                    setDiscipline('all');
        ...
        <div className="flex items-center justify-between text-xs text-slate-400 px-1">
          <p>
            Showing <span className="text-white font-mono font-bold">{filtered.length}</span> verified creative professionals
          </p>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="py-20 text-center glass-panel rounded-3xl border border-sky-500/20 p-8 space-y-4">
            ...
            <p className="text-xs text-slate-400 max-w-md mx-auto">
              No creative professionals matched your search parameters. Try adjusting your filters or search keywords.
            </p>
            <button
              onClick={() => {
                setSearch('');
